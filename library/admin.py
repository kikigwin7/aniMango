    ...
    def manual_request(self, request, queryset):
        ...
        if user:
            for item in queryset:
                ...
                if r:
                    r.approve()
                    self.message_user(request, 'Successfully approved loan for {0!s}'.format(item.__str__()),
                                      messages.SUCCESS)
                else:
...
class RequestAdmin(admin.ModelAdmin):
        ...
        for request_obj in queryset:
            if request_obj.approve():
                messages.add_message(request, messages.SUCCESS,
                                     'Successfully approved loan for {0!s}'.format(request_obj.item.__str__()))
            else:
                messages.add_message(request, messages.ERROR,
                                     'Could not approve loan for {0!s}'.format(request_obj.item.__str__()))
    ...
    def deny(self, request, queryset):
        for request_obj in queryset:
            if request_obj.deny():
                messages.add_message(request, messages.SUCCESS,
                                     'Successfully denied loan for {0!s}'.format(request_obj.item.__str__()))
            else:
                messages.add_message(request, messages.ERROR,
                                     'Could not deny loan for {0!s}'.format(request_obj.item.__str__()))
    ...
    def absent(self, request, queryset):
        for request_obj in queryset:
            if request_obj.absent():
                messages.add_message(request, messages.SUCCESS,
                                     'Marked as available {0!s}'.format(request_obj.item.__str__()))
            else:
                messages.add_message(request, messages.ERROR,
                                     'Could not mark as available {0!s}'.format(request_obj.item.__str__()))
    ...
    def return_on_time(self, request, queryset):
        for request_obj in queryset:
            if request_obj.returned('Returned'):
                messages.add_message(request, messages.SUCCESS,
                                     'On time return of {0!s}'.format(request_obj.item.__str__()))
            else:
                messages.add_message(request, messages.ERROR,
                                     'Could not return {0!s}'.format(request_obj.item.__str__()))
    ...
    def return_late(self, request, queryset):
        for request_obj in queryset:
            if request_obj.returned('Late'):
                messages.add_message(request, messages.SUCCESS,
                                     'Late return of {0!s}'.format(request_obj.item.__str__()))
            else:
                messages.add_message(request, messages.ERROR,
                                     'Could not return {0!s}'.format(request_obj.item.__str__()))
